...
def process_blob_upload(input_blob: blob.BlobClient, processed_container: blob.ContainerClient) -> None:
    ...
    blob_name =  input_blob.get_blob_properties().name #.split('/')[-1]
    file_size = input_blob.get_blob_properties().size
    ...
    try:
        ...
        processed_container.upload_blob(processed_blob_name, input_blob.download_blob(), overwrite=True)
